...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == ADMIN_ID:
        ...
        msg_text = (
            f"👮‍♂️ **管理员控制台**\n"
            f"🔔 提醒设置：到期前 {notify_days} 天\n"
            f"🗑 自动清理：过期后 {cleanup_days} 天"
        )
        keyboard = [
...
async def client_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "client_buy_new":
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("🔙 重选套餐", callback_data="client_buy_new" if order_type == 'new' else f"selrenew_{target_uuid}")]
        ]
...
async def admin_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_plans_list":
        ...
        db_execute("INSERT INTO plans VALUES (?, ?, ?, ?, ?, ?)", 
                   (key, new_plan['name'], new_plan['price'], new_plan['days'], new_plan['gb'], strategy))
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == ADMIN_ID and 'add_plan_step' in context.user_data and text:
        ...
        if step == 'name':
            ...
            keyboard = [
                [InlineKeyboardButton("🚫 永不重置", callback_data="set_strategy_NO_RESET")],
                [InlineKeyboardButton("📅 每日重置", callback_data="set_strategy_DAY")],
                [InlineKeyboardButton("🗓 每周重置", callback_data="set_strategy_WEEK")],
                [InlineKeyboardButton("🌝 每月重置", callback_data="set_strategy_MONTH")],
    ...
    if user_id in temp_orders and text:
        ...
        kb = [[InlineKeyboardButton("✅ 通过", callback_data=f"ap_{user_id}_{order['plan']}_{order['type']}_{safe_uuid}")], 
              [InlineKeyboardButton("❌ 拒绝", callback_data=f"rj_{user_id}")]]
